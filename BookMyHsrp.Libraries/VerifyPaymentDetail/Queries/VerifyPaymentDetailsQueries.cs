...
namespace BookMyHsrp.Libraries.VerifyPaymentDetail.Queries
{
    public class VerifyPaymentDetailsQueries
    {
        ...
        public static readonly string GetBookingHistoryId = "select BookingHistoryID from [BookMyHSRP].dbo.Appointment_BookingHist where VehicleRegNo = @RegistrationNo and right(trim(@ChassisNo),5) = @ChassisNo.Substring(@ChassisNo.Length - 5) and right(trim(@Engineno),5) = @Engineno.Substring(@Engineno.Length - 5) and OrderStatus in ('Success','Shipped','Success-Test') and PlateSticker='plate' and OrderType=@OrderType.Trim().ToUpper() ";
        ...
        public static readonly string GetDataBetweenElse = "select top 1 case when getdate() Between OrderClosedDate And DATEADD(DAY, 7, OrderClosedDate) then 'N' else 'Y' end ReBookingAllow,OrderClosedDate, Vehicleregno from hsrprecords WITH (NOLOCK) where Vehicleregno =@RegistrationNo and right(trim(Chassisno),5) = @ChassisNoo and  OrderClosedDate<>'' order by OrderClosedDate desc";
